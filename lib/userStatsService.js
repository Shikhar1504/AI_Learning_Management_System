...
export class UserStatsService {
  ...
  static async getUserStats(userEmail) {
    try {
      ...
      // Sync completedCourses with live data if needed
      if (completedCoursesWithProgress !== (user.completedCourses || 0)) {
        await db
          .update(USER_TABLE)
          .set({ 
            completedCourses: Math.round(completedCoursesWithProgress),
            // Update overall progress as well since it depends on completedCourses
