...
export class UserStatsService {
  ...
  static async getUserStats(userEmail) {
    try {
      ...
      const [userData, userCourses] = await Promise.all([
        db.select().from(USER_TABLE).where(eq(USER_TABLE.email, userEmail)).limit(1),
        db.select().from(STUDY_MATERIAL_TABLE).where(eq(STUDY_MATERIAL_TABLE.createdBy, userEmail))
      ]);
      ...
      const completedCoursesWithProgress = await this.calculateCompletedCoursesWithProgress(userEmail, userCourses);
    ...
    try {
      ...
      if (Object.keys(updates).length > 0) {
        ...
        for (const [key, value] of Object.entries(updates)) {
          ...
          const integerFields = ['streak', 'studyTime', 'completedCourses', 'progress', 'experiencePoints', 'learnerLevel', 'levelProgress', 'dailyCoursesCreated'];
  ...
  static async calculateCompletedCoursesWithProgress(userEmail, courses) {
    try {
      ...
      const { CHAPTER_NOTES_TABLE, STUDY_TYPE_CONTENT_TABLE } = await import('@/configs/schema');
      const { eq, and, count } = await import('drizzle-orm');
      
      // Filter only Ready courses (generating courses can't have progress)
      const readyCourses = courses.filter(course => 
      ...
      for (const course of readyCourses) {
        try {
          ...
          const hasFlashcards = studyMaterialsResult.some(item => item.type.toLowerCase() === 'flashcard');
          const hasQuiz = studyMaterialsResult.some(item => item.type.toLowerCase() === 'quiz');
          ...
          console.warn(`Failed to calculate progress for course ${course.courseId}:`, error.message);
      ...
      console.error('Error calculating completed courses with progress:', error);
  ...
  static calculateOverallProgress(user, courseStats) {
    ...
    const courseProgress = Math.min(100, 
  ...
  static getProgressBreakdown(user, courseStats) {
    return {
      courses: {
        value: Math.round(Math.min(100, (courseStats.total * 10) + (user.completedCourses * 20))),
  ...
  static calculateGoalProgress(user, courseStats) {
    ...
    const progressPercentage = goal.target ? 
      Math.min(100, Math.round((currentProgress / goal.target) * 100)) : 0;
  ...
  static processActivity(user, activity) {
    ...
    if (activity.studyTimeHours) {
      updates.studyTime = Math.round((user.studyTime || 0) + activity.studyTimeHours);
    }
  ...
  static processDirectUpdates(user, updates) {
    ...
    const allowedFields = [
      'streak', 'studyTime', 'completedCourses', 'progress',
      'currentGoal', 'preferences', 'lastStudyDate'
    ];

    allowedFields.forEach(field => {
      if (updates[field] !== undefined) {
        // Ensure numeric fields are properly converted to integers
        if (['streak', 'studyTime', 'completedCourses', 'progress'].includes(field)) {
  ...
  static isActivity(data) {
    const activityFields = [
      'courseCreated', 'courseCompleted', 'chaptersCompleted',
      'quizzesCompleted', 'flashcardSessions', 'studyTimeHours',
      'dailyStreak', 'weeklyStreak', 'firstLogin', 'profileCompleted',
      'goalAchieved', 'dailyActivity'
    ];
  ...
  static canCreateCourse(user) {
    ...
    if (user.dailyCoursesCreated === undefined || user.lastCourseDate === undefined) {
      return true;
    }
  ...
  static getTodayCoursesUsed(user) {
    ...
    const today = new Date();
    const lastCourseDate = user.lastCourseDate ? new Date(user.lastCourseDate) : null;
  ...
  static async incrementDailyCourseCount(userEmail) {
    try {
      ...
      if (currentUser.dailyCoursesCreated === undefined || currentUser.lastCourseDate === undefined) {
        console.log('Daily course tracking columns not found, allowing course creation');
        return {
          success: true,
      ...
      const today = new Date();
      const lastCourseDate = currentUser.lastCourseDate ? new Date(currentUser.lastCourseDate) : null;
      ...
      if (dailyCount > maxDailyCourses) {
        throw new Error(`Daily course limit reached. Free users can create ${maxDailyCourses} courses per day.`);
      }
