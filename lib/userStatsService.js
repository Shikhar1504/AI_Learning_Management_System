...
export class UserStatsService {
  ...
  static async getUserStats(userEmail) {
    ...
    while (retries > 0) {
      try {
        ...
        userData = results[0];
        userCourses = results[1];
        break; // Success, exit retry loop
      } catch (dbError) {
        retries--;
        console.warn(`[UserStats] DB fetch failed, retrying... (${retries} left). Error: ${dbError.message}`);
        if (retries === 0) {
          console.error("Error getting user stats:", dbError);
          throw new Error("Failed to retrieve user statistics due to database connection error");
        }
    ...
    try {
      ...
      const completedCoursesWithProgress =
        this.calculateCompletedCoursesWithProgress(userCourses);

      // Calculate topic statistics
      const topicStats = this.calculateTopicStats(userCourses);

      // Calculate overall progress
      const overallProgress = this.calculateOverallProgress(user, courseStats);

      // Log a discrepancy warning but do NOT write from a read path.
      // The update should happen in the write path (e.g. after notes generation completes),
      // not silently inside a GET endpoint — that makes GET non-idempotent.
      if (completedCoursesWithProgress !== (user.completedCourses || 0)) {
        console.warn(`[UserStats] completedCourses mismatch for ${userEmail}: stored=${user.completedCourses}, live=${completedCoursesWithProgress}`);
      }
      ...
      return {
        ...
        dailyCoursesCreated: this.getTodayCoursesUsed(user),
        ...
        completedTopics: topicStats?.completedTopics || 0,
      };
    } catch (error) {
  ...
  static calculateCourseStats(courses) {
    ...
    // We need to fetch this from the database for accuracy, but as a fallback/optimization
    // we can try to estimate or we need to update the query in getUserStats to include this data.
    // For now, let's initialize them to 0 and let the calling function or a separate query handle it if needed
    // OR better, let's just add the query to getUserStats to be robust.
   ...
   * @returns {Promise<number>} Number of completed courses (100% progress)
   */
  static calculateCompletedCoursesWithProgress(courses) {
    if (!courses || courses.length === 0) return 0;

    return courses.filter((course) => (course.progressPercentage || 0) === 100)
      .length;
  }

  /**
   * Calculate total and completed topics for a user
   * @param {string} userEmail - User email address
   * @returns {Promise<Object>} Topic statistics { totalTopics, completedTopics }
   */
  static calculateTopicStats(courses) {
  ...
  static calculateStreakInfo(user) {
    ...
    if (lastStudyDate) {
      ...
      const lastMidnight = new Date(lastStudyDate);
      lastMidnight.setHours(0, 0, 0, 0);
      
      const diffTime = todayMidnight.getTime() - lastMidnight.getTime();
      const daysSinceLastStudy = Math.floor(diffTime / (1000 * 60 * 60 * 24));
      ...
      if (daysSinceLastStudy > 1) {
        ...
        // daysSinceLastStudy <= 0 (Today or Future)
        status = "active";
        daysUntilLoss = 2; // Can skip today (already studied) and still be active tomorrow
      }
    } else if (streak === 0) {
      status = "none";
    }
  ...
  static updateStreak(user) {
    ...
    // Directional difference: today - last
    const diffTime = todayMidnight.getTime() - lastMidnight.getTime();
    
    // Use floor to ensure full integer days
    const daysSinceLastStudy = Math.floor(diffTime / (1000 * 60 * 60 * 24));

    if (daysSinceLastStudy < 0) {
      // Future date - treat as same day, just update timestamp
      return {
        lastStudyDate: today,
      };
    } else if (daysSinceLastStudy === 0) {
      // Already studied today (same calendar day), no change to streak
      // But update lastStudyDate to keep time current
      return {
        lastStudyDate: today,
      };
    } else if (daysSinceLastStudy === 1) {
