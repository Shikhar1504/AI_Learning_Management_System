...
function Practice() {
      ...
      if (response.ok) {
        ...
        if (data.questions && data.questions.length > 0) {
          ...
          setIsCorrect(null);
            ...
            toast.error("No practice questions available yet. Create some courses first!");
        }
      }
    } catch (error) {
  ...
  const handleOptionSelect = (option) => {
    ...
    if (isAnswerCorrect) {
      ...
      toast.success("Correct!", { position: 'bottom-center', duration: 1000 });
       ...
       toast.error(`Incorrect. Correct answer: ${currentQuestion.answer}`, { position: 'bottom-center', duration: 2000 });
    }
  };
  ...
  const getFeedbackMessage = (percentage) => {
    if (percentage >= 90) return { title: "Excellent!", message: "You're a master of these topics.", color: "text-green-500" };
    if (percentage >= 70) return { title: "Great Job!", message: "Solid understanding, keep it up.", color: "text-blue-500" };
    if (percentage >= 50) return { title: "Good Effort", message: "You're getting there. Review weaker topics.", color: "text-yellow-500" };
    return { title: "Keep Practicing", message: "Review your course materials and try again.", color: "text-red-500" };
  };
        ...
        <div className="glass-card p-6 md:p-8 rounded-2xl border border-white/10 shadow-xl animate-in fade-in slide-in-from-bottom-4 duration-500">
           ...
           <div className="grid gap-4">
             {currentQuestion?.options?.map((option, idx) => {
               ...
               let optionClass = "p-4 rounded-xl border-2 text-left transition-all duration-200 font-medium ";
               ...
               if (selectedOption !== null) {
                  ...
                  // Normal state
                  optionClass += "bg-white/5 border-transparent hover:bg-white/10 hover:border-purple-500/50 hover:shadow-lg active:scale-[0.99]";
               }
  ...
  if (quizCompleted) {
    const percentage = questions.length > 0 ? Math.round((score / questions.length) * 100) : 0; // ISSUE 2 FIX: Prevent div by zero
    ...
    return (
      <div className="max-w-2xl mx-auto text-center space-y-8 py-10">
        <div className="glass-card p-8 md:p-12 border border-white/10 rounded-3xl shadow-2xl animate-in zoom-in-95 duration-500">
           ...
           <h2 className={`text-4xl font-bold mb-2 font-display ${feedback.color}`}>
             {feedback.title}
           </h2>
           ...
           <div className="grid grid-cols-3 gap-6 mb-10">
              ...
              <div className="p-4 rounded-2xl bg-white/5 border border-white/10">
                 <div className="text-3xl font-bold text-purple-400 mb-1">{formatTime(timer)}</div>
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold font-display text-foreground">Practice Arena</h1>
          <p className="text-muted-foreground text-lg">Master your knowledge with randomized quizzes.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: "Total Attempts", value: stats.totalAttempts, icon: Target, color: "text-blue-400", bg: "bg-blue-500/10" },
          { label: "Best Score", value: `${stats.bestScore}%`, icon: Trophy, color: "text-yellow-400", bg: "bg-yellow-500/10" },
          { label: "Average Score", value: `${stats.averageScore}%`, icon: Zap, color: "text-purple-400", bg: "bg-purple-500/10" },
          { label: "Last Score", value: `${stats.lastScore}%`, icon: Clock, color: "text-pink-400", bg: "bg-pink-500/10" },
        ].map((stat, idx) => (
          <Card key={idx} className="glass-card border-white/10 hover:border-white/20 transition-all">
      ...
      <div className="grid md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 relative overflow-hidden rounded-3xl bg-gradient-to-br from-indigo-900/50 to-purple-900/50 border border-white/10 p-8 md:p-10 shadow-2xl">
           ...
           <div className="relative z-10 flex flex-col items-start h-full justify-center">
              ...
              <h2 className="text-3xl font-bold text-white mb-4 font-display">Mixed Practice Quiz</h2>
        ...
        <div className="glass-card rounded-3xl border border-white/10 p-6 flex flex-col justify-between">
           ...
           <div className="mt-6 pt-6 border-t border-white/10">
              <div className="text-xs text-muted-foreground font-medium uppercase tracking-wider mb-2">Current Streak</div>
              <div className="flex items-end gap-2">
                 <span className="text-3xl font-bold text-foreground">
                    {/* Placeholder for streak integration if needed, could use actual streak */}
                    --
                 </span>
