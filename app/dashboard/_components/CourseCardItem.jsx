...
function CourseCardItem({ course, onCourseUpdate }) {
  ...
  const fetchCourseAnalytics = useCallback(async (retryCount = 0) => {
    ...
    try {
      ...
      if (response.data.fallback) {
        console.warn('📊 Using fallback analytics data for course:', course.courseId);
        if (response.data.ultraFast) {
          console.warn('⚡ Using ultra-fast fallback for course:', course.courseId);
        }
      }
    } catch (error) {
      ...
      if (axios.isCancel(error)) {
        console.log('Analytics request cancelled for course:', course.courseId);
        return;
      }
  ...
  const checkCourseStatus = useCallback(async () => {
    ...
    try {
      ...
      if (course?.status === "Generating" && updatedCourse?.status === "Ready") {
  ...
  }, [course?.courseId, course?.createdBy, course?.status, fetchCourseAnalytics, onCourseUpdate]);
  ...
  const calculateOverallProgress = () => {
    ...
    const flashcardProgress = (analytics.hasFlashcards || analytics.materialCounts?.flashcard > 0) ? 30 : 0;
    const quizProgress = (analytics.hasQuiz || analytics.materialCounts?.quiz > 0) ? 30 : 0;
    ...
    <div className="modern-card-interactive group relative overflow-hidden h-full">
      ...
      <div className="p-6 h-full flex flex-col">
        ...
        <div className="flex items-start justify-between mb-6">
          <div className={`p-3 rounded-xl bg-gradient-to-br ${getStatusColor()}/20 group-hover:scale-110 transition-transform duration-300`}>
            {isCompleted ? (
            ...
            <div className="space-y-3">
              ...
              {!loadingAnalytics && progressPercentage > 0 && (
                <div className="flex justify-center gap-3 text-xs text-gray-500">
                  <span className="flex items-center gap-1" title="Chapter Progress">
                    📚 {Math.round(((analytics.completedChapters || 0) / (analytics.totalChapters || 1)) * 40)}%
                  </span>
                  <span className="flex items-center gap-1" title="Flashcards">
                    🃏 {(analytics.hasFlashcards || analytics.materialCounts?.flashcard > 0) ? '30%' : '0%'}
                  </span>
                  <span className="flex items-center gap-1" title="Quiz">
                    ❓ {(analytics.hasQuiz || analytics.materialCounts?.quiz > 0) ? '30%' : '0%'}
                  </span>
                  {analytics.fallback && (
                    <span className="flex items-center gap-1 text-orange-400" title="Using fallback data">
                      ⚠️
                    </span>
                  )}
                </div>
              ...
              <div className="space-y-2">
                ...
                <div className="bg-orange-500/10 border border-orange-500/20 rounded-md p-3">
                  <div className="flex items-start gap-2 text-xs text-orange-300">
                    ...
                    <div>
                      <p className="font-medium mb-1">Please wait patiently...</p>
                      <p className="text-orange-400/80">Content generation typically takes 5-10 minutes. The page will refresh automatically once complete.</p>
                    </div>
                  </div>
                </div>
              </div>
