...
function Quiz() {
  ...
  const trackStudyActivity = useCallback(async () => {
    ...
    try {
      ...
      // Then track activity with a short retry loop for transient failures.
      ...
      for (let attempt = 1; attempt <= 3; attempt++) {
        try {
          await axios.put(`/api/users/${userEmail}/stats`, payload);
          lastError = null;
          break;
        } catch (err) {
          lastError = err;
  ...
  const handleAnswerSelect = (userAnswer) => {
    ...
    // Check if answer is correct (normalized to avoid AI hallucinations with whitespace/case)
    const normalize = (str) =>
      String(str || "")
        .trim()
        .toLowerCase();
  ...
  if (quiz.length > 0) {
        ...
        <div className="min-h-screen bg-background">
          ...
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            <div className="text-center space-y-8">
              <div
                className={`w-32 h-32 rounded-full mx-auto flex items-center justify-center ${isGoodScore ? "bg-green-500/20" : "bg-orange-500/20"}`}
              >
    ...
    // Normalize to prevent whitespace/case check errors in UI rendering
    const normalize = (str) =>
      String(str || "")
        .trim()
        .toLowerCase();
                  ...
                  <div className="flex items-start gap-4">
                    <div
                      className={`p-2 rounded-lg ${
                        isSelectionCorrect ? "bg-green-500/20" : "bg-red-500/20"
                      }`}
                    >
                      {isSelectionCorrect ? (
                    ...
                    <div className="flex-1">
                      <h3
                        className={`font-semibold mb-2 ${
                          isSelectionCorrect ? "text-green-400" : "text-red-400"
                        }`}
                      >
    ...
    <div className="min-h-screen bg-background flex items-center justify-center">
      <div className="text-center space-y-6 max-w-md">
        ...
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground">
            There is no quiz for this course yet.
          </p>
        </div>
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          <Button
