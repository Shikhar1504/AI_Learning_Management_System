...
function MaterialCardItem({ item, studyTypeContent, course, refreshData, loading: globalLoading }) {
  ...
  return (
    <div className="group relative h-full min-h-[420px] rounded-3xl bg-card border border-white/5 overflow-hidden transition-all duration-500 hover:shadow-[0_0_40px_-10px_rgba(124,58,237,0.3)] hover:-translate-y-2">
      {/* Gradient Border via Pseudo-element */}
      <div className={`absolute inset-0 rounded-3xl p-[1px] bg-gradient-to-br ${item.gradient} opacity-0 group-hover:opacity-100 transition-opacity duration-500`} style={{ maskImage: 'linear-gradient(black, black) content-box, linear-gradient(black, black)', maskComposite: 'exclude' }} />
      ...
      <div className="relative p-6 h-full flex flex-col z-10">
        ...
        <div className="flex items-center justify-center mb-8">
          <div className={`px-4 py-1.5 rounded-full text-xs font-semibold uppercase tracking-wider border transition-all duration-300 ${
            ...
            <div className="flex items-center gap-2">
              ...
              <span>
                {isGenerated || item.type === 'notes' ? 'Ready' : 'Not Generated'}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col items-center text-center space-y-6">
          {/* Material Image */}
          <div className={`relative p-5 rounded-3xl bg-gradient-to-br ${item.bgGradient} group-hover:scale-105 transition-all duration-500 shadow-xl group-hover:shadow-2xl ring-1 ring-white/10 group-hover:ring-white/20`}>
            ...
            {/* Glow effect */}
            <div className={`absolute inset-0 rounded-3xl bg-gradient-to-r ${item.gradient} opacity-0 group-hover:opacity-30 blur-xl transition-opacity duration-700`} />
          </div>

          {/* Title & Description */}
          <div className="space-y-3 px-2">
            <h3 className="text-xl font-bold text-white group-hover:text-transparent group-hover:bg-gradient-to-r group-hover:bg-clip-text group-hover:from-purple-300 group-hover:to-blue-300 transition-all duration-500">
              {item.name}
            </h3>
          ...
          {isGenerated && (
            <div className="w-full pt-2">
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-white/5 border border-white/5 text-sm md:text-xs">
                  {item.type === 'flashcard' && <span className="text-purple-300">🃏 {itemCount} Cards</span>}
                  {item.type === 'quiz' && <span className="text-green-300">❓ {itemCount} Questions</span>}
                  {item.type === 'notes' && <span className="text-blue-300">📝 {itemCount} Chapters</span>}
              </div>
            </div>
          )}
        </div>
        ...
        <div className="mt-8">
          {isGenerated || item.type === 'notes' ? (
            <Link href={`/course/${course?.courseId}${item.path}`} className="block w-full">
              <Button className="w-full h-12 rounded-xl bg-gradient-to-r from-white/10 to-white/5 hover:from-purple-500 hover:to-blue-500 text-white border border-white/10 hover:border-transparent transition-all duration-300 group-hover:shadow-lg group-hover:shadow-purple-500/20">
                <span className="font-semibold tracking-wide mr-2 group-hover:mr-3 transition-all">Start Learning</span>
            ...
            >
              {isGenerating ? (
                ...
                <>
                  ...
                  <span className="group-hover:text-white transition-colors">Generate Content</span>
                </>
              )}
            </Button>
          )}
