...
function CourseSidebar({ course, topics, activeTopic, setActiveTopic, className }) {
  ...
  return (
    <div className={cn("h-full border-r border-border bg-card/30 backdrop-blur-sm overflow-y-auto w-80 flex-shrink-0", className)}>
      ...
      <div className="p-4">
        <Accordion type="multiple" defaultValue={chapters.map((_, i) => `item-${i}`)} className="w-full space-y-4">
              ...
              <AccordionItem key={index} value={`item-${index}`} className="border-none">
                <AccordionTrigger className="hover:no-underline py-2 px-3 rounded-lg hover:bg-accent/50 group">
                  <div className="flex items-start text-left gap-3">
                    <span className="text-2xl group-hover:scale-110 transition-transform">{chapter.emoji}</span>
                    <div>
                    ...
                    {chapterTopics.map((topic) => (
                      ...
                      >
                         {topic.status === "completed" ? (
                           <CheckCircle className={cn("h-4 w-4 flex-shrink-0", activeTopic?.id === topic.id ? "text-primary" : "text-green-500/70")} />
