...
function TopicLearning() {
  ...
  const GetCourseAndTopics = async () => {
    try {
      ...
      const courseData = courseResp.data.result;
  ...
  useEffect(() => {
    const fetchContent = async () => {
      ...
      if (activeTopic.notesContent || activeTopic.status !== 'completed') return;
    ...
    try {
      ...
      if (result.data?.content) {
        ...
        const updatedTopics = topics.map((t) => {
          if (t.id === topicId) {
            return { ...t, status: "completed", notesContent: result.data.content };
          }
          return t;
        });
        ...
        if (activeTopic?.id === topicId) {
          setActiveTopic({ ...activeTopic, status: "completed", notesContent: result.data.content });
        }
  ...
  if (loading && !course) { // Only show full screen loader if course is not loaded
    return (
