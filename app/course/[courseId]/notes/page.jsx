...
function ViewNotes() {
  ...
  const cleanNotesContent = (htmlContent) => {
    if (!htmlContent) return "";

    let cleaned = htmlContent
      .replace(/<body[^>]*>/gi, "<div>") // Replace opening body with div
      .replace(/<\/body>/gi, "</div>") // Replace closing body with div
      .replace(/<html[^>]*>/gi, "")
      .replace(/<\/html>/gi, "")
      .replace(/<head[^>]*>[\s\S]*?<\/head>/gi, "");

    // Scope CSS to prevent body interference
    const cssMatches = cleaned.match(/<style[^>]*>([\s\S]*?)<\/style>/gi);
    if (cssMatches) {
      cssMatches.forEach((styleTag) => {
        const cssContent = styleTag.replace(/<\/?style[^>]*>/gi, "");
        const scopedCSS = cssContent
          .replace(/\bbody\s*{/g, ".noteClass > div {") // Scope body styles to the container
          .replace(/\bbody\b/g, ".noteClass") // Replace other body references
          .replace(/\bhtml\b/g, ".noteClass");

        cleaned = cleaned.replace(styleTag, `<style>${scopedCSS}</style>`);
      });
    }
        ...
        <div className="w-full max-w-4xl mt-4">
          {!notes ? (
            ...
            <div
              className="noteClass border p-6 md:p-8 rounded-2xl bg-card"
              style={{
                // Prevent styles from leaking to parent
                isolation: "isolate",
